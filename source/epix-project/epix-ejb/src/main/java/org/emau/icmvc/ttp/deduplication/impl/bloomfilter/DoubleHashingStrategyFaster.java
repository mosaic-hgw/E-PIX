 ...
 */
public class DoubleHashingStrategyFaster extends DoubleHashingStrategy
{
	/**
	 ...
	 */
	public DoubleHashingStrategyFaster(int length, int iterations, int nGramLength)
	{
		super(length, iterations, nGramLength);
	}
	...
	protected int getHashedPosition(String value, String salt, int index)
	{
		...
		int relevantBytes = (int) Math.ceil(Math.log(getLength()) / Math.log(256));

		int h1 = 0;
		int h2 = 0;
		for (int i = 0, j = hash1.length - 1, k = hash2.length - 1; i < relevantBytes; ++i, --j, --k)
		{
			int shift = i * 8;
			h1 += (hash1[j] + 128) << shift;
			h2 += (hash2[k] + 128) << shift;
		}

		return (h1 + index * h2) % getLength();
	}
}
