 ...
 */
public class JaccardSimilarityAlgorithmCoded implements IStringMatchingAlgorithm
{
	private static final Logger logger = LogManager.getLogger(JaccardSimilarityAlgorithmCoded.class);

	/**
	 * Returns the jaccard similarity of two encoded bloom filters {@link BloomFilter#getAsEncodedString}.
	 *
	 * @param str1 First encoded bloom filter
	 * @param str2 Second encoded bloom filter
	 * @throws NumberFormatException if length is not correct formatted
	 *
	...
	public double match(String str1, String str2)
	{
		...
		for (int i = 0; i < str1.length(); ++i)
		{
			...
			or += Base64Cache.getUnionCardinality(a, b);
		}

		return (or != 0) ? (double) and / (double) or : 1.0;
	}
